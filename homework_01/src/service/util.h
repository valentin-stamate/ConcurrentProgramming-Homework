...
public:
    ...
    static void writeTo(int protocol, int fd, void* buffer, int bufferSize, sockaddr_in &addr, int* packageCount, int* bytesCount);
    static void readFrom(int protocol, int fd, void* buffer, int bufferSize, sockaddr_in &addr, int* packageCount, int* bytesCount);
};
